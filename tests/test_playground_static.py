...
def test_playground_helper_exposes_contact_actions():
    ...
    assert "sendTestMessage:" in helper
    ...
    assert "/setup/test-message" in helper
...
def test_playground_has_guided_add_friend_panel():
    ...
    assert "Send test message" in markup
    ...
    assert 'id="sendTestMessageButton"' in markup
    ...
    assert "LeftLevelApi.sendTestMessage" in script
    assert "test friend created" in script
    assert "test message delivered locally" in script
